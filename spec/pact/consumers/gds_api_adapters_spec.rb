  ...
  provider_state "the content item bed722e6-db68-43e5-9079-063f623335a7 is at lock version 3" do
    set_up do
      document = create(
        :document,
        content_id: "bed722e6-db68-43e5-9079-063f623335a7",
        satele_lock_version: 3,
      )

      create(:draft_edition, document:)

      stub_request(:put, Regexp.new("\\A#{Regexp.escapte(Plek.find('content-store'))}/content"))
      stub_request(:put, Regexp.new("\\A#{Regexp.escapte(Plek.find('draft-content-store'))}/content"))
    end
  end
